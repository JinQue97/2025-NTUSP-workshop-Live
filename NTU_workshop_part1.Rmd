...
## Fit test 2 (fitting of 2 data points)
```{r}
test_last_two_dataset <- busulfan_dataset %>% 
  group_by(ID) %>%
  filter(row_number() == 1 | row_number() == n()-1 | row_number() == n()) %>% ungroup()

# Plasma drug concentration decline in two phases shown in semi-log plot
ggplot(test_last_two_dataset, aes(TIME/60, DV, group = ID, color = as.factor(ID))) +
  geom_point() +
  geom_line() +
  theme_bw() +
  labs(title = "Busulfan Test Dose semi-log plot - Two Last Points", x = "Time (hours)", y = "Conc (mg/L)") +
  theme(legend.position = "blank")

# Model Fitting
test_two_last_two_cmt_final_pk_fit <- nlmixr2(
  busulfan_final_2cmt_model,
  test_last_two_dataset,
  "posthoc",
  control = foceiControl(maxOuterIterations=0),
  table = list(cwres = TRUE, npde = TRUE)
)
...
## Individual estimation with full data points 
test_two_last_data <- test_two_last_two_cmt_final_pk_fit$origData
test_two_last_indv_cl <- data.frame(ID = test_two_last_two_cmt_final_pk_fit$ID, Clearance = test_two_last_two_cmt_final_pk_fit$cl)

test_two_last_indiv_AUC<- merge(test_two_last_data, test_two_last_indv_cl, by = 'ID')
test_two_last_indiv_AUC <- test_two_last_indiv_AUC %>% mutate(AUC = AMT/Clearance) %>% select(ID, AUC)

test_two_last_indiv_AUC <- test_two_last_indiv_AUC %>%
  group_by(ID) %>%
  summarize(last_two_tp_AUC = first(AUC), .groups = "drop")

test_two_last_indiv_AUC
```
...
```{r}
test_compare_auc <- merge(test_full_indiv_AUC, test_two_last_indiv_AUC, by = "ID")
...
df_test_long$Method <- factor(df_test_long$Method, levels = c("full_estimation_AUC", "last_two_tp_AUC", "last_tp_AUC"))
...
wrap_plots(p_two_tp, p_last, ncol = 2)
```
