...
```{r}
# Plasma drug concentration decline in two phases shown in semi-log plot
busulfan_last_two_tp_dataset <- busulfan_First_Dose_dataset %>% 
  group_by(ID) %>%
  filter(row_number() == 1 | row_number() == n()-1 | row_number() == n()) %>% ungroup()
  
ggplot(busulfan_last_two_tp_dataset, aes(TIME/60, DV, group = ID, color = as.factor(ID))) +
  geom_line() +
  geom_point() +
  theme_bw() +
  labs(title = "Busulfan First Dose semi-log plot", x = "Time (hours)", y = "Conc (mg/L)") +
...
two_cmt_last_two_tp_pk_fit <- nlmixr2(
  busulfan_final_2cmt_model,
  busulfan_last_two_tp_dataset,
  "posthoc",
  control = foceiControl(maxOuterIterations=0),
  table = list(cwres = TRUE, npde = TRUE)
)
...
## Last Two Time Point
orig_data <- two_cmt_last_two_tp_pk_fit$origData
indv_cl <- data.frame(ID = two_cmt_last_two_tp_pk_fit$ID, Clearance = two_cmt_last_two_tp_pk_fit$cl)

last_two_tp_indiv_exposure <- merge(orig_data, indv_cl, by = 'ID')
last_two_tp_indiv_exposure <- last_two_tp_indiv_exposure %>% mutate(AUC = AMT/Clearance) %>% select(ID, AUC)

last_two_tp_AUC <- last_two_tp_indiv_exposure %>%
  group_by(ID) %>%
  summarize(last_two_AUC = first(AUC), .groups = "drop")
...
## Fit First 1
```{r}
last_time_point <- busulfan_last_two_tp_dataset %>% 
  group_by(ID) %>%
...
```{r}
compare_auc <- merge(full_tp_AUC, last_two_tp_indiv_AUC, by = "ID")
...
df_long$Method <- factor(df_long$Method, levels = c("full_tp_AUC", "last_two_AUC", "last_tp_AUC", "no_tp_AUC"))
...
df_last_two <- compare_auc %>%
  select(ID, Full = full_tp_AUC, Two_TP = last_two_AUC) %>%
  pivot_longer(cols = -ID, names_to = "Method", values_to = "AUC") %>%
...
# --- Full vs No TP
df_no <- compare_auc %>%
  select(ID, Full = full_tp_AUC, No_Obs = no_tp_AUC) %>%
  pivot_longer(cols = -ID, names_to = "Method", values_to = "AUC") %>%
